...
int main(int argc, char **argv) {
    ros::init(argc, argv, "rh_commander"); 
    ...
    //start new motion
    twist_cmd.angular.z=yaw_rate; //spinning to a horizontal way
    twist_cmd.linear.x=0.0; //

    timer=0.0; //reset the timer
    while(timer<1.59*time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }   
    ...
    twist_cmd.angular.z=0.0; 
    twist_cmd.linear.x=speed; // moving forward

    timer=0.0; //reset the timer
    while(timer<2.2*time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }  

    twist_cmd.angular.z=-yaw_rate; //spinning around 60 degree
    twist_cmd.linear.x=0.0; //
    ...
    twist_cmd.angular.z=0.0; //
    twist_cmd.linear.x=speed; //moving forward again
    ...
    twist_cmd.angular.z=-yaw_rate; //spinning towards the destination
    twist_cmd.linear.x=0.0; //

    timer=0.0; //reset the timer
    while(timer<0.55*time_3_sec) {
          twist_commander.publish(twist_cmd);
          timer+=sample_dt;
          loop_timer.sleep();
          }  
    ...
    twist_cmd.angular.z=0.0; //
    twist_cmd.linear.x=speed; //moving towards the destination
