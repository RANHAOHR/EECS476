...
bool callback(rh_serv_pkg::PathSrvRequest& request, rh_serv_pkg::PathSrvResponse& response)
{
ROS_INFO("callback activated");
  ...
  /*  /////////////////////////
       if (i>0){
              last_pose_desired = request.nav_path.poses[i-1].pose;
              travel_distance = pow(pose_desired.position.x - last_pose_desired.position.x,2) +pow(pose_desired.position.y - last_pose_desired.position.y,2);
              travel_distance = sqrt(travel_distance);
              
           }
        else{
            travel_distance = pose_desired.position.x;
